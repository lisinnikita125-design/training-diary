...
# ══════════════════════════════════════════════
PROGRESS_PERIOD_DAYS = {"1m": 30, "3m": 90, "6m": 182, "1y": 365, "all": 36500}
...
def progress_summary():
    """
    Индекс силы: медиана % изменения e1RM (медиана первой половины периода
    против медианы второй половины периода) по упражнениям, у которых есть
    минимум PROGRESS_MIN_WINDOW_SESSIONS сессий в обеих половинах. Окна
    масштабируются с длиной периода (1М — 15д/15д, 1Г — ~6мес/~6мес), а не
    фиксированы, чтобы на длинных периодах хватало данных для редко
    тренируемых упражнений. Плюс тоннаж/тренировки/рекорды/список
    ...
    # Окна масштабируются с длиной периода: первая половина периода против
    # второй половины (не фикс. 2 недели) — иначе на длинных периодах (6М/1Г)
    # в узкий 14-дневный срез почти никогда не набирается PROGRESS_MIN_WINDOW_SESSIONS
    # сессий для упражнений вне самой частой ротации.
    period_mid = period_start + (period_end - period_start) / 2
    first_window_end = period_mid   # исключительно
    last_window_start = period_mid  # исключительно
    ...
    # (разгрузка/техническая тренировка), случайно попавшей в окно. Но при
    # ровно 2 значениях медиана = среднему, поэтому ниже
    # PROGRESS_MIN_WINDOW_SESSIONS сессий в окне % вообще не считаем.
