...
def ai_analyze():
    ...
    prompt = (
        ...
        "- 1-2 упражнения где падали повторения в последних подходах, предположи причину.\n\n"
        "**🩺 Восстановление**\n"
        "- Риск перетренированности: низкий/средний/высокий.\n"
        "- Если выше среднего — конкретная рекомендация.\n\n"
        "**📈 Прогресс за 30 дней**\n"
        "- 2-3 упражнения с наибольшим приростом.\n"
        "- 1-2 упражнения где прогресс застопорился.\n"
        "- Итог по тоннажу: растёт/стабилен/падает.\n\n"
        "**📋 Веса на следующую тренировку**\n"
        "Таблица: Упражнение | Текущий вес | Рекомендуемый вес | Причина\n"
        "Правила: все подходы с запасом → +2.5кг; последние тяжело → держать или -2.5кг; пропущено → пропущено.\n\n"
        "**💡 Итог**\n"
        "- Один вердикт (увеличивать/восстановление/снижать).\n"
        "- Один конкретный совет на завтра.\n\n"
        "**⚖️ Финальный вердикт**\n- Посмотри на таблицу: если 50%+ упражнений повышают вес → 'Сегодня работаем в полную силу'. Если 50%+ держат или снижают → 'Снижаем интенсивность на X%'. Вердикт ДОЛЖЕН логически следовать из таблицы.\n- Один практический совет на СЕГОДНЯ: конкретное действие (например 'съешь 30г белка в течение часа после тренировки' или 'спи 8 часов — мышцы растут во сне'). Без общих слов.\n\nВАЖНО: все веса в таблице кратны 5. Таблицу форматируй строго в markdown: | Упражнение | Текущий | Рекомендуемый | Причина |\nБудь краток. Только цифры и конкретика."
    )
