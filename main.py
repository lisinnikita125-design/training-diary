...
def ai_analyze():
    ...
    prompt = (
        ...
        recovery + "\n\n"
        "ПРАВИЛА ОТВЕТА:\n"
        "1. ПЕРСОНАЛИЗАЦИЯ ОБЯЗАТЕЛЬНА: используй возраст, пол и вес из профиля минимум один раз в анализе. Пример: 'Для мужчины 31 года с массой 78 кг текущий темп прогрессии безопасен.'\n"
        "2. Формулируй уверенно. Не 'может быть усталость' — а 'причина: падение повторений в последних подходах указывает на накопленную усталость'.\n"
        "3. ЗАПРЕЩЕНО использовать слова 'легко', 'тяжело', 'с запасом' если в данных нет оценки сложности. Вместо этого — факты: 'повторения оставались стабильными', 'повторения снизились на X в последнем подходе'.\n"
