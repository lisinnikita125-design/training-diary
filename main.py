...
@app.route("/ai-workout-history")
def ai_workout_history():
    require_auth()
    uid = current_user_id()
    conn = get_db()
    rows = conn.execute("""
...
def ai_history():
    ...
    rows = conn.execute("""
        SELECT id, summary_text,
               strftime('%d.%m.%Y %H:%M', created_at, '+5 hours') as formatted_date,
               created_at
    ...
    """, (uid,)).fetchall()
    conn.close()
    history = [{"id": r["id"], "text": r["summary_text"], "date": r["formatted_date"]} for r in rows]
