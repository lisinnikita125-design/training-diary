...
def ai_analyze():
    ...
    prompt = (
        ...
        "(день 1: верх, день 2: ноги+талия, день 3: верх). Цели: рост силы, контроль техники, профилактика травм." + (" " + profile_str if profile_str else "") + "\n\n"
        ...
        recovery + "\n\n"
        "ПРАВИЛА ОТВЕТА:\n"
        "1. Используй данные профиля (возраст, пол, вес) в тексте явно — например: 'Для атлета X лет с массой Y кг...'.\n"
        "2. Формулируй уверенно. Не 'может быть усталость' — а 'причина: накопленная усталость, так как повторения упали в последних подходах'.\n"
        "3. Вердикт ДОЛЖЕН совпадать с таблицей весов: если 50%+ упражнений снижают/держат вес — вердикт 'восстановление', не прогрессия.\n"
        "4. Практический совет — персональный, не шаблонный. Учитывай профиль, самочувствие, длительность тренировки.\n\n"
        ...
        "**🎯 Индекс готовности**\n"
        "Таблица: Показатель | Оценка\n"
        "Строки: Выполнение подходов (X/10) | Восстановление (X/10) | Стабильность нагрузки (X/10) | Общая готовность (X/10)\n"
